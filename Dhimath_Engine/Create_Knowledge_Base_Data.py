...
def df_processing(df, df_name):
    ...
    for idx, row in df.iterrows():
        ...
        formatted_chunk = "".join([f"{key}: {value}\n" for key, value in chunk.items()])
...
def create_knowledge_base_data(global_session_vars):  
    ...
    if process_step in ["Auto Learn"]:
        data_sheets_df = df_check(global_session_vars["data_sheets_df"])
        text_chunks = df_processing(data_sheets_df, file)
